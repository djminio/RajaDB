...
//Ban blueprint.
/*1.int isBanned(const char *player_name)
* 2.need to add a return 0; so it doesnt keep blocking the id;
* 3.add some short of variable to check if the player is ok to login
* 4. add a variable for player ip;
* 5.add a variable for player name;
* 6. need to copy the memory with memset e.g memset(variable,variable,value,value);
* 7.a variable for the query to be held in char szquery1[255];
* 8. need to add HSTMT variable for selecting the players name in the table/rows.
* 9. need to also add a print function to the rajadb so we can see if it works or not.
* 10. need to add sqlexecdirect
* 11.if statement if the query was successfull or not.
...
void ban_player(const char *player_name)
{
	int checkStatus = 0;
	int player_ip[40];
	memset(player_ip, 0, 40);

	char szquery[255];
	HSTMT Hstmt = NULL;

	//print result to dbdemon
	sprintf(szquery, "select * from ban_list where [name]='%s'", player_name);
...
int IsBlockedId(const char *check_name)
{
	...
	char	szQuerry[255];
	HSTMT hStmt = NULL; //repeatedly for a SELECT query since you want to retrieve the query result

	sprintf(szQuerry, "select * from Black_List where [Name] ='%s'", check_name);//prints the results to the dbdemon
	...
	{
		retCode = SQLFetch(hStmt);
		// the code below checks if the player name is recorded if not then it will log them in
		switch (retCode)
		{
		...
		default://if the name is written it will then stop you from logging in.
		{
			char Name[20];
	...
	{//checks if the table and rows are correct if not then it will print out the following error bellow.
		MyLog(0, "!!!!--------------------------------------------------!!!!");
		MyLog(0, "DragonDB in BlockedId Table Error!!!  PLZ Check DB Table");
		MyLog(0, "!!!!--------------------------------------------------!!!!");
	}

	SQLFreeStmt(hStmt, SQL_DROP);//stops the query when it reaches the end.
	return 0;
}
